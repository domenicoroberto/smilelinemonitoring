...
/// ✅ SERVIZIO SEMPLIFICATO: Timer persistente con salvataggio ogni secondo
/// LOGICA SEMPLICE:
/// - daily_seconds_today = contatore giornaliero
/// - timer_start_time = ora di inizio (solo se running)
/// - timer_is_running = true/false
/// - Salva daily_seconds OGNI SECONDO in SharedPreferences
class BackgroundTimerService {
  ...
  Future<void> pauseTimer() async {
    try {
      ...
      // ✅ CRITICO: NON ricalcolare da timer_start_time!
      // Il valore è già aggiornato OGNI SECONDO da _startSecondlySave()
      // Se ricalcoli, conti 2 volte!

      final currentDaily = getDailySeconds();

      print('⏸️ PAUSA: Salvati $currentDaily secondi');
  ...
  /// ⚠️ CRITICO: daily_seconds include già i secondi aggiornati ogni secondo da _startSecondlySave()
  /// NON ricalcolare da timer_start_time, quello creerebbe doppi conteggi!
  int getTotalSeconds() {
    try {
      // ✅ BASTA! Ritorna solo daily_seconds
      // _startSecondlySave() aggiorna già questo valore ogni secondo
  ...
  Future<bool> checkDayChanged() async {
    try {
      ...
      print('🌙 CHECK GIORNO:');
      print('   - lastCheckStr: $lastCheckStr');
      print('   - today: $today');

      // ✅ Se è la PRIMA volta (lastCheckStr == null), salva oggi e ritorna FALSE
      if (lastCheckStr == null) {
        print('   - PRIMA VOLTA: salvo il giorno odierno');
        await _prefs.setString('last_day_check', today);
        return false; // ← Non è un cambio giorno, è il primo giorno!
      }

      // ✅ Se è diverso da oggi, è cambiato il giorno
      if (lastCheckStr != today) {
        print('   - ✅ CAMBIO GIORNO RILEVATO: era $lastCheckStr, oggi è $today');
        await _prefs.setString('last_day_check', today);
        return true;
      }

      // ✅ Stesso giorno
      print('   - STESSO GIORNO: $today');
      return false;
    } catch (e) {
  ...
  Future<void> resetForNewDay() async {
    try {
      ...
      // qui resettiamo solo SharedPreferences
